...
<div class="row wrapper border-bottom white-bg page-heading">
	<div class="col-lg-8">
		<br>
		<span class="label label-primary" style="font-size:12px">- Impact -</span>
		<h2>{{$basic->name}}</h2>
	</div>
	<div class="col-lg-2">
...
<script>
	$(document).ready(function() {
		...
		$(".eventlist a").click(function(ev) {
			...
			$("body").mask("");
			$.get("/ajax/project/impact/events/get/"+$id, {}, function(response) {
				$("body").unmask();
				if(response.success) {
					info = response.info;
		...
		$("button[name='modal-join']").click(function() {
			...
			/*
			$.post("/ajax/project/impact/events/join/"+$id, {
				...
				if(response.success) {
					...
					$("button[name='withdraw_event']").click(function() {
						...
						$.post("/ajax/project/impact/events/withdraw/"+$id, {}, function(response) {
							if(response.success) {
		...
		$("button[name='invite']").click(function() {
			...
			$.post("/ajax/project/impact/{{$project_id}}/"+$id+"/invite", {}, function(response) {
				if(response.success) {
