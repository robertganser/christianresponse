...
<div class="row wrapper border-bottom white-bg page-heading">
	<div class="col-lg-8">
		<br>
		<span class="label label-primary" style="font-size:12px">- Prayer -</span>
		<h2>{{$basic->name}}</h2>
	</div>
	<div class="col-lg-2">
					...
					<div class="portlet">
						<div class="portlet_wrapper">
							...
							<div class="portlet_title">
								<h4 style="color: #d18022"><b><span itemprop="name">{{$basic->name}}</title></b></h4>
							</div>
...
<script>
	$(document).ready(function() {
		...
		$(".eventlist a").click(function(ev) {
			...
			$("body").mask("");
			$.get("/ajax/project/prayer/events/get/"+$id, {}, function(response) {
				$("body").unmask();
				if(response.success) {
					info = response.info;
		...
		$("button[name='modal-join']").click(function() {
			...
			$.post("/ajax/project/prayer/events/join/"+$id, {
				...
				if(response.success) {
					...
					$("button[name='withdraw_event']").click(function() {
						...
						$.post("/ajax/project/prayer/events/withdraw/"+$id, {}, function(response) {
							if(response.success) {
		...
		$("button[name='invite']").click(function() {
			...
			$.post("/ajax/project/prayer/{{$project_id}}/"+$id+"/invite", {}, function(response) {
				if(response.success) {
