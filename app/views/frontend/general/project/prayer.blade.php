			...
			<div class="ibox">
				<div class="ibox-title" style="display:table;width:100%;text-align: right">
					...
					<button type="button" name="new" class="btn btn-primary">
						<i class="fa fa-newspaper-o"></i>&nbsp;New Prayer Project
					</button>
					&nbsp;
				...
				<div class="ibox-content">
					<div class="table-responsive m-t">
						<table class="table">
							...
							<tbody>
								<?php $i = 1; foreach($prayers as $one) :?>
									<tr>
										...
										<td nowrap=""><div class="rating-mark" data-score="<?php echo $one->review?>"></div></td>
										<td nowrap="">
											<?php if($one->total_donation_amount > 0) :?>
												<a href="/{{$active=='manages'?'manages/':''}}projects/prayer/{{$one->id}}/transactions"><i class="fa fa-dollar"></i> {{number_format($one->total_donation_amount, 2)}}</a>
											<?php endif;?>
										</td>
										<td nowrap="" width="100px">
											<?php if($one->event_count > 0) :?>
												<a href="/{{$active=='manages'?'manages/':''}}projects/prayer/{{$one->id}}/events"><span class="label label-info"><?php echo $one->event_count?></span></a>
											<?php else:?>
												<a href="/{{$active=='manages'?'manages/':''}}projects/prayer/{{$one->id}}/events"><span class="label label-default">manage</span></a>
											<?php endif;?>
										</td>
										<td nowrap="" width="100px">
											<?php if($one->follow_count > 0) :?>
												<a href="/{{$active=='manages'?'manages/':''}}projects/prayer/{{$one->id}}/followings"><span class="label label-danger"><?php echo $one->follow_count?></span></a>
											<?php endif;?>
										</td>
										<td nowrap="" width="100px">
											<?php if($one->hug_count > 0) :?>
												<a href="/{{$active=='manages'?'manages/':''}}projects/prayer/{{$one->id}}/hugs"><span class="label label-warning">{{$one->hug_count}}</span></a>
											<?php endif;?>
										</td>
										<td nowrap="" align="right" width="70px">
											<?php if($one->status != -2) : ?>
												<button type="button" name="edit-row" data-id="<?php echo $one->id?>" class="btn btn-xs btn-outline btn-primary">
...
<form name="frmPrayer" method="POST">
...
<script>
	$(document).ready(function() {
		$("button[name='delete']").click(function() {
			...
			$("form[name='frmPrayer'] input[name='selected_id']").val($id);
			$("form[name='frmPrayer']").submit();
		...
		$("button[name='agree']").click(function() {
			location.href = "/projects/prayer/edit";
		});

		$("button[name='edit-row']").click(function() {
			var id = $(this).attr("data-id");
			location.href = "/projects/prayer/edit/" + id;
		});
	}); 
</script>
@stop