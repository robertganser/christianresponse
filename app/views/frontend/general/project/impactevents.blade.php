...
<div class="row wrapper border-bottom white-bg page-heading">
	<div class="col-lg-10">
		...
		<ol class="breadcrumb">
			...
			<li>
				<a href="/{{$active=='manages'?'manages/':''}}projects/impact"><?php echo $project_title?></a>
			</li>
			<li class="active">
		...
		<br>
		<span class="label label-info">Impact Project</span>
	</div>
	<div class="col-lg-2">
							...
							<tbody>
								<?php $i = 1; foreach($events as $one) :?>
									<tr>
										...
										<td nowrap=""><a href="/{{$active=='manages'?'manages/':''}}projects/impact/{{$project_id}}/event/{{$one->id}}/joins"><span class="label label-info">{{$one->joined_count}}</span></a></td>
										<td nowrap="" align="right" width="70px">
											<a href="/{{$active=='manages'?'manages/':''}}projects/impact/{{$project_id}}/events/edit/{{$one->id}}">
...
<form name="frmImpactEvents" method="POST">
...
<script>
	$(document).ready(function() {
		...
		$("button[name='agree']").click(function() {
			location.href = "/{{$active=='manages'?'manages/':''}}projects/impact/{{$project_id}}/events/edit";
		});
		...
		$("button[name='delete']").click(function() {
			...
			$("form[name='frmImpactEvents'] input[name='selected_id']").val($id);
			$("form[name='frmImpactEvents']").submit();
