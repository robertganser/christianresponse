							...
							<tbody>
								<?php $i = 1; foreach($prayers as $one) :?>
									<tr>
										...
										<td nowrap="">
											<a href="/projects/prayer/{{$one->id}}/reviews">
												<div class="rating-mark" data-score="<?php echo $one->review?>"></div>
											</a>
										</td>
										<td nowrap="">
											<?php if($one->total_donation_amount > 0) :?>
												<a href="/projects/prayer/{{$one->id}}/transactions"><i class="fa fa-dollar"></i> {{number_format($one->total_donation_amount, 2)}}</a>
											<?php endif;?>
										</td>
										<td nowrap="" width="100px">
											<?php if($one->event_count > 0) :?>
												<a href="/projects/prayer/{{$one->id}}/events"><span class="label label-info"><?php echo $one->event_count?></span></a>
											<?php else:?>
												<a href="/projects/prayer/{{$one->id}}/events"><span class="label label-default">manage</span></a>
											<?php endif;?>
										</td>
										<td nowrap="" width="100px">
											<?php if($one->follow_count > 0) :?>
												<a href="/projects/prayer/{{$one->id}}/followings"><span class="label label-danger"><?php echo $one->follow_count?></span></a>
											<?php endif;?>
										</td>
										<td nowrap="" width="100px">
											<?php if($one->hug_count > 0) :?>
												<a href="/projects/prayer/{{$one->id}}/hugs"><span class="label label-warning">{{$one->hug_count}}</span></a>
											<?php endif;?>
										</td>
