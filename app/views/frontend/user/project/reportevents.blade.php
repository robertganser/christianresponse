							...
							<tbody>
								<?php $i = 1; foreach($events as $one) :?>
									<tr>
										...
										<td nowrap=""><a href="/projects/report/{{$type}}/{{$project_id}}/event/{{$one->id}}/joins"><span class="label label-info">{{$one->joined_count}}</span></a></td>
										<td nowrap="" align="right" width="70px">
											<a href="/projects/report/{{$type}}/{{$project_id}}/events/edit/{{$one->id}}">
...
<script>
	$(document).ready(function() {
		...
		$("button[name='agree']").click(function() {
			location.href = "/projects/report/{{$type}}/{{$project_id}}/events/edit";
		});
