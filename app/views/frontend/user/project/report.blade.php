							...
							<tbody>
								<?php $i = 1; foreach($reports as $one) :?>
									<tr>
										...
										<td nowrap="">
											<a href="/projects/report/{{$one->project_type == 1 ? 'nationalreport' : 'regionalreport'}}/{{$one->id}}/reviews">
												<div class="rating-mark" data-score="<?php echo $one->review?>"></div>
											</a>
										</td>
										<td nowrap="">
											<?php if($one->total_donation_amount > 0) :?>
												<a href="/projects/report/{{$one->project_type == 1 ? 'nationalreport' : 'regionalreport'}}/{{$one->id}}/transactions"><i class="fa fa-dollar"></i> {{number_format($one->total_donation_amount, 2)}}</a>
											<?php endif;?>
										</td>
										<td nowrap="" width="100px">
											<?php if($one->event_count > 0) :?>
												<a href="/projects/report/{{$one->project_type == 1 ? 'nationalreport' : 'regionalreport'}}/{{$one->id}}/events"><span class="label label-info">{{$one->event_count}}</span></a>
											<?php else:?>
												<a href="/projects/report/{{$one->project_type == 1 ? 'nationalreport' : 'regionalreport'}}/{{$one->id}}/events"><span class="label label-default">manage</span></a>
											<?php endif;?>
										</td>
										<td nowrap="" width="100px">
											<?php if($one->follow_count > 0) :?>
												<a href="/projects/report/{{$one->project_type == 1 ? 'nationalreport' : 'regionalreport'}}/{{$one->id}}/followings"><span class="label label-danger">{{$one->follow_count}}</span></a>
											<?php endif;?>
										</td>
										<td nowrap="" width="100px">
											<?php if($one->hug_count > 0) :?>
												<a href="/projects/report/{{$one->project_type == 1 ? 'nationalreport' : 'regionalreport'}}/{{$one->id}}/hugs"><span class="label label-warning">{{$one->hug_count}}</span></a>
											<?php endif;?>
										</td>
										<td nowrap="" align="right" width="70px">
											<?php if($one->status != -2) : ?>
												<a href="/projects/report/{{$one->project_type == 1 ? 'nationalreport' : 'regionalreport'}}/edit/{{$one->id}}">
	...
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<div class="modal-footer" align="center" style="text-align: center">
				<a href="/projects/report/nationalreport/edit" class="btn btn-info" name="choice_nationalreport">
					National Report
				</a>
				<a href="/projects/report/regionalreport/edit" class="btn btn-warning" name="choice_regionalreport">
					Regional Report
				</a>
			</div>
		</div>
