			...
			<div class="ibox">
				<div class="ibox-title" style="display:table;width:100%;text-align: right">
					...
					<button type="button" name="new" class="btn btn-primary">
						<i class="fa fa-newspaper-o"></i>&nbsp;New Impact Project
					</button>
					&nbsp;
				...
				<div class="ibox-content">
					<div class="table-responsive m-t">
						<table class="table">
							...
							<tbody>
								<?php $i = 1; foreach($impacts as $one) :?>
									<tr>
										...
										<td nowrap="">
											<a href="/{{$active=='manages'?'manages/':''}}projects/impact/{{$one->id}}/reviews">
												<div class="rating-mark" data-score="<?php echo $one->review?>"></div>
											</a>
										</td>
										<td nowrap="">
											<?php if($one->total_donation_amount > 0) :?>
												<a href="/{{$active=='manages'?'manages/':''}}projects/impact/{{$one->id}}/transactions"><i class="fa fa-dollar"></i> {{number_format($one->total_donation_amount, 2)}}</a>
											<?php endif;?>
										</td>
										<td width="100px" nowrap="">
											<?php if($one->event_count > 0) :?>
												<a href="/{{$active=='manages'?'manages/':''}}projects/impact/{{$one->id}}/events"><span class="label label-info"><?php echo $one->event_count?></span></a>
											<?php else:?>
												<a href="/{{$active=='manages'?'manages/':''}}projects/impact/{{$one->id}}/events"><span class="label label-default">manage</span></a>
											<?php endif;?>
										</td>
										<td width="100px" nowrap="">
											<?php if($one->follow_count > 0) :?>
												<a href="/{{$active=='manages'?'manages/':''}}projects/impact/{{$one->id}}/followings"><span class="label label-danger"><?php echo $one->follow_count?></span></a>
											<?php endif;?>
										</td>
										<td width="100px" nowrap="">
											<?php if($one->hug_count > 0) :?>
												<a href="/{{$active=='manages'?'manages/':''}}projects/impact/{{$one->id}}/hugs"><span class="label label-warning">{{$one->hug_count}}</span></a>
											<?php endif;?>
										</td>
										<td align="right" width="70px" nowrap="">
											<?php if($one->status != -2) : ?>
												<a href="/{{$active=='manages'?'manages/':''}}projects/impact/edit/{{$one->id}}">
...
<form name="frmImpact" method="POST">
...
<script>
	$(document).ready(function() {
		$("button[name='delete']").click(function() {
			...
			$("form[name='frmImpact'] input[name='selected_id']").val($id);
			$("form[name='frmImpact']").submit();
		...
		$("button[name='agree']").click(function() {
			location.href = "/projects/impact/edit";
		});
	}); 
</script>
@stop