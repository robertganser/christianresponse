						...
						<table class="table">
							...
							<tbody>
								<?php $i = 1; foreach($events as $one) :?>
									<tr>
										<td nowrap=""><input type="checkbox" name="chk[]" value="{{$one->id}}" data-type="{{$type}}"></td>
										<td nowrap="">{{$i}}</td>
										...
										<td nowrap=""><a href="/{{$active=='manages'?'manages/':''}}projects/report/{{$type}}/{{$project_id}}/event/{{$one->id}}/joins"><span class="label label-info">{{$one->joined_count}}</span></a></td>
										<td nowrap="" align="right" width="70px">
											<a href="/{{$active=='manages'?'manages/':''}}projects/report/{{$type}}/{{$project_id}}/events/edit/{{$one->id}}">
												<button type="button" name="edit-row" class="btn btn-xs btn-outline btn-primary">
													<i class="fa fa-edit"></i>
												</button>
											</a>
											<button type="button" name="delete-row" data-id="{{$one->id}}" data-type="{{$type}}" class="btn btn-xs btn-outline btn-danger">
...
<form name="frmReportEvents" method="POST">
...
<script>
	$(document).ready(function() {
		$("button[name='delete']").click(function() {
			...
			$("form[name='frmReportEvents'] input[name='selected_id']").val($id);
			$("form[name='frmReportEvents']").submit();
		...
		$("button[name='agree']").click(function() {
			location.href = "/{{$active=='manages'?'manages/':''}}projects/report/{{$type}}/{{$project_id}}/events/edit";
		});
