...
<div class="row wrapper border-bottom white-bg page-heading">
	<div class="col-lg-10">
		...
		<ol class="breadcrumb">
			...
			<li>
				<a href="/{{$active=='manages'?'manages/':''}}projects/prayer"><?php echo $project_title?></a>
			</li>
			<li class="active">
		...
		<br>
		<span class="label label-info">Prayer Project</span>
	</div>
	<div class="col-lg-2">
							...
							<tbody>
								<?php $i = 1; foreach($events as $one) :?>
									<tr>
										...
										<td nowrap=""><a href="/{{$active=='manages'?'manages/':''}}projects/prayer/{{$project_id}}/event/{{$one->id}}/joins"><span class="label label-info">{{$one->joined_count}}</span></a></td>
										<td nowrap="" align="right" width="70px">
											<a href="/{{$active=='manages'?'manages/':''}}projects/prayer/{{$project_id}}/events/edit/{{$one->id}}">
...
<form name="frmPrayerEvents" method="POST">
...
<script>
	$(document).ready(function() {
		$("button[name='delete']").click(function() {
			...
			$("form[name='frmPrayerEvents'] input[name='selected_id']").val($id);
			$("form[name='frmPrayerEvents']").submit();
		...
		$("button[name='agree']").click(function() {
			location.href = "/{{$active=='manages'?'manages/':''}}projects/prayer/{{$project_id}}/events/edit";
		});
